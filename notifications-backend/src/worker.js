      ...
      if(url.pathname.startsWith("/api/admin/")){
        ...
        if(request.method==="GET"&&url.pathname==="/api/admin/liberte"){
          const [requestResult,exceptionResult,userResult]=await Promise.all([
            env.DB.prepare(`SELECT id,user_id,name,email,date,status,comment,created_at,updated_at FROM paddock_requests ORDER BY date DESC,id DESC`).all(),
          ...
          return json({requests:requestResult.results.map(publicPaddockRequest),exceptions:exceptionResult.results.map(row=>({date:row.date,open:Boolean(row.is_open),comment:row.comment||"",updatedAt:row.updated_at})),users:userResult.results.map(publicUser)},200,cors);
        }
