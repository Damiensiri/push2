...
test('fondations chevaux sur D1 : migration, permissions, conservation, concurrence et budgets',async t=>{
 ...
 await t.test('planning client agrégé, horaires facultatifs et droits selon la source',async()=>{
  ...
  await call('/api/admin/planning/horses',{method:'POST',body:{weekStart:'2026-09-07',horseId:id}});
  const privatePlanning=await call('/api/admin/planning?week=2026-09-07');assert.ok(!privatePlanning.data.tasks.some(task=>task.id===taskId));assert.ok(!privatePlanning.data.events.some(event=>event.source==='client'));
  assert.equal((await call('/api/admin/planning/tasks/'+taskId,{method:'PATCH',body:{completed:true}})).status,404);
 ...
 await t.test('publication PWA : défaut OFF, exceptions, bascule, lots et conservation',async()=>{
  ...
  const linked=await create({type:'paddock',paddock:'Maison',startsAt:'10:00',endsAt:'11:00',requestId:request.id});assert.equal(linked.pwaAutomatic,true);assert.equal((await events()).length,2);
