...
test('la PWA expose Mes chevaux avec une fiche et un planning agrégé',async()=>{
  ...
  assert.doesNotMatch(page+script,/adminNotes|horse_owners|photo_key|notifications-beta/i);
  const version=home.match(/const APP_VERSION="([^\"]+)"/)[1];
  for(const text of [worker,manifest])assert.ok(text.includes(version));
});
